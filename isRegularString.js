...
function checkStringType(str) {
  if (isBase64(str)) {
    ...
    console.log(`${str} is a Regular string.`);
  } else if (!isNaN(Number(str))) {
    console.log(`${str} is a Numeric value.`);
  } else if (str.toLowerCase() === 'true' || str.toLowerCase() === 'false') {
    console.log(`${str} is a Boolean value.`);
  } else if (str.toLowerCase() === 'null') {
    console.log(`${str} is a Null value.`);
  } else {
    console.log(`${str} is of unknown type.`);
  }
}

// Test with examples
const regularString = "Prakasam";
const base64EncodedString = "";
const numericValue = "885620230802110638454";
const booleanValue = "true";
const nullValue = "null";

checkStringType(regularString);
checkStringType(base64EncodedString);
checkStringType(numericValue);
checkStringType(booleanValue);
checkStringType(nullValue);
