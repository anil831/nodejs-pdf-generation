//code for convert and attach image to pdf
const express =require('express');

const app = express();

const fs = require('fs');
const PDFDocument = require('pdfkit');
const path = require("path");

function createPDF(data) {
  const doc = new PDFDocument({ size: 'A4', margin: 50 });


  const cellPadding = 8;
  const headerHeight = 30;
  const pageHeight = doc.page.height;
  const gapAfterTitle=60;
  let y = headerHeight+gapAfterTitle;
  let currentPage = 1;

  //new variables
  const pageWidth = doc.page.width - doc.page.margins.left - doc.page.margins.right;
  const keyCellWidth = pageWidth / 4;

  function drawTableCell(x, y, width, height, text,isHeader) {
    if (typeof text === 'string' && checkStringType(text)==="unknown") {
      const decodedImage = Buffer.from(text, 'base64');
      const outputPath = path.join(__dirname, `output_aadhaar.jpg`);
      fs.writeFileSync(outputPath, decodedImage);
      doc
        .rect(doc.page.margins.left+x -5, y, width, height)
        .stroke()
      doc.image(outputPath, doc.page.margins.left+x + cellPadding -5, y + cellPadding, { width: 100, height: 100,align:"right" }); // Adjust image size as needed
    } else {
        if(isHeader){
            doc.font('Helvetica-Bold');
        }else{
            doc.font('Helvetica')
        }
      doc
        .rect(doc.page.margins.left+x-5, y, width, height)
        .stroke().fontSize(9)
        .text(text, doc.page.margins.left+x + cellPadding-5, y + cellPadding, {
          width: width - 2 * cellPadding,
          height: height - 2 * cellPadding,
          align: 'left',
          valign: 'center'
        });
        
    }
  }
  

  function drawTableRow(firstKey,secondKey,firstValue,secondValue) {
    
    const initialX = cellPadding;

    let firstKeyHeight=0,secondKeyHeight=0,firstValueHeight=0,secondValueHeight=0;

    if(checkStringType(firstValue)==="unknown" || checkStringType(secondValue)==="unknown"){
        
        firstKeyHeight = 120;
        secondKeyHeight = 120;
        firstValueHeight = 120;
        secondValueHeight = 120;
    }else{

        firstKeyHeight = doc.heightOfString(firstKey, { width: keyCellWidth - cellPadding * 2 });
        secondKeyHeight = doc.heightOfString(firstValue, { width: keyCellWidth - cellPadding * 2 });
        firstValueHeight = doc.heightOfString(secondKey, { width: keyCellWidth - cellPadding * 2 });
        secondValueHeight = doc.heightOfString(secondValue, { width: keyCellWidth - cellPadding * 2 });
    }


    const rowHeight = Math.max(firstKeyHeight, secondKeyHeight, firstValueHeight, secondValueHeight) + cellPadding * 2;


    // Check if the row exceeds the page height
    if (y + rowHeight + cellPadding > pageHeight) {
      doc.addPage();
      currentPage++;
      y = headerHeight;
    }

    drawTableCell(initialX, y, keyCellWidth, rowHeight, firstKey,true);

    drawTableCell(initialX + keyCellWidth, y, keyCellWidth, rowHeight, firstValue);

    drawTableCell(initialX + 2* keyCellWidth, y, keyCellWidth, rowHeight, secondKey,true);

    drawTableCell(initialX + 3* keyCellWidth, y, keyCellWidth, rowHeight, secondValue);

    y += rowHeight;
  }

    // Add content to the PDF
    doc.fontSize(18).text('Aadhaar Details', { align: 'center' });
    doc.moveDown();

  y += headerHeight;

  
  const keys = Object.keys(data);
for (let i = 0; i < keys.length; i += 2) {
  const firstKey = keys[i];
  const secondKey = keys[i + 1];
  const firstValue = data[firstKey];
  const secondValue = data[secondKey];

      drawTableRow(firstKey,secondKey,firstValue,secondValue);

}



  // Save the PDF on the Node.js server
  const fileName = 'output9.pdf';
  doc.pipe(fs.createWriteStream(fileName));
  doc.end();

  console.log(`PDF saved as ${fileName}`);
}





















const data = {
    aadhaar_reference_id: "885620230801163333124",
    aadhaar_download_timestamp: "2023-08-01 16:33:33",
    aadhaar_xml_age: "0",
    aadhaar_xml_version: "1.0",
    aadhaar_name:
      "Tanneeru Anil Kumar",
    aadhaar_gender: "M",
    aadhaar_dob: "10-08-1996",
    aadhaar_age: "26",
    aadhaar_care_of: "S/O Hanumantha Rao",
    aadhaar_address:
      "S/O Hanumantha Rao, 7-249/2, sujatha nagar 4th line, Ongole, Ongole, Prakasam, Andhra Pradesh, India, 523001",
    aadhaar_house: "7-249/2",
    aadhaar_street: "sujatha nagar 4th line",
    aadhaar_landmark: "",
    aadhaar_location: "",
    aadhaar_post_office: "Ongole",
    aadhaar_district: "Prakasam",
    aadhaar_sub_district: "",
    "aadhaar_photo": "",
    aadhaar_vtc: "Ongole",
    aadhaar_state: "Andhra Pradesh",
    aadhaar_country: "India",
    aadhaar_post_code: "523001",
    aadhaar_last_four_digits: "8856",
    aadhaar_mobile: null,
    aadhaar_email: null,
  };

  createPDF(data);

app.listen(8080,(err)=>{
    if(err) console.log(err);
})





function isBase64(str) {
  try {
    // Decoding the input string from Base64
    const decodedData = Buffer.from(str, 'base64').toString('utf-8');

    // If the decoded data matches the original input, then it is Base64-encoded
    return decodedData === str;
  } catch (error) {
    // If decoding fails, it is not Base64-encoded
    return false;
  }
}

function isRegularString(str) {
  // Allow letters, digits, space, and common punctuation marks.
  const regularStringRegex = /^[A-Za-z0-9\s.,!?@#$%^&*()_+-=[\]{}|\\;:'"<>/]*$/;
  return regularStringRegex.test(str) && str?.length <= 256;
}


function checkStringType(str) {
  if (isBase64(str)) {
    return "Base64-encoded";
  } else if (isRegularString(str)) {
    return "Regular";

  } else if (!isNaN(Number(str))) {
    return "Numeric";

  } else if (str?.toLowerCase() === 'true' || str?.toLowerCase() === 'false') {
    return "Boolean";

  } else if (str?.toLowerCase() === 'null') {
    return "Null";

  } else {
    return "unknown";

  }
}
