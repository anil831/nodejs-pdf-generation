...
function createPDF(data) {
    ...
    function addPageNumber() {
      ...
      doc.text(`Page ${page}`, 300, 760,{continued:true});
      doc.text(`confidential`, 50, 760).link(50, 760, doc.widthOfString("confidential"), 20, linkURL);
    ...
    tableData.forEach((row) => {
        const rowHeight = drawTableRow(row); // Get rowHeight from the function
      
        // Check if the row exceeds the page height
        if (y + rowHeight + cellPadding > pageHeight) {
            addPageNumber(); // Add the page number to the new page
...
const data = {
  ...
  function addWatermark(doc, text) {
    const watermarkFont = 'Helvetica-Bold'; // You can choose the font and size
    const watermarkFontSize = 48; // Adjust the font size as needed
    const watermarkOpacity = 0.3; // Adjust the opacity as needed
  
    const textWidth = doc.widthOfString(text, { font: watermarkFont, size: watermarkFontSize });
    const textHeight = doc.heightOfString(text, { font: watermarkFont, size: watermarkFontSize });
  
    const pageWidth = doc.page.width;
    const pageHeight = doc.page.height;
  
    const diagonalLength = Math.sqrt(pageWidth * pageWidth + pageHeight * pageHeight);
    const angle = Math.atan(pageHeight / pageWidth);
    const x = pageWidth / 2 - (diagonalLength * Math.cos(angle) / 2);
    const y = pageHeight / 2 - (diagonalLength * Math.sin(angle) / 2);
  
    doc.opacity(watermarkOpacity)
      .font(watermarkFont)
      .fontSize(watermarkFontSize)
      .rotate(-angle, { origin: [x, y] }) // Rotate text diagonally
      .fillColor('red') // Adjust the color as needed
      .text(text, x, y);
    doc.opacity(1); // Restore the opacity for other content
  }
